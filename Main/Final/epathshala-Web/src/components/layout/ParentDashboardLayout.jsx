...
function ParentDashboardLayout({ children }) {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => {
    const stored = localStorage.getItem('parentSidebarCollapsed');
    return stored === 'true';
  });
  ...
  const handleSidebarCollapse = () => {
    setSidebarCollapsed((prev) => {
      localStorage.setItem('parentSidebarCollapsed', !prev);
      return !prev;
    });
  };
  ...
  return (
    <Box 
      className="parent-dashboard-container"
      sx={{ 
        display: 'flex', 
        minHeight: '100vh',
        width: '100%',
